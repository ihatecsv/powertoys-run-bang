...
foreach ($platform in "ARM64", "x64")
{
    ...
    dotnet build $PSScriptRoot\PowerToys-Run-Bang.sln -c Debug /p:Platform=$platform

    Remove-Item -Path "$PSScriptRoot\Community.PowerToys.Run.Plugin.Bang\bin\*" -Recurse -Include *.xml, *.pdb, PowerToys.*, Wox.*
    Rename-Item -Path "$PSScriptRoot\Community.PowerToys.Run.Plugin.Bang\bin\$platform\Debug" -NewName "Bang"
